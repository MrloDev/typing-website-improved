...
function App() {
    ...
    const end = () => { 
        ...
        document.getElementById('history').innerHTML += `<div id='historyEl'>${wpmString} WPM  / ${wordAmount} Words  /  ${endTime / 10} Seconds</div>`
    ...
    const checkWord = (event) => {  
        if (!hasEnded) {
            ...
            document.getElementById(wordIndex).style.color = "#a8dadc"; 
                ...
                if (event.code === 'Backspace') { 
                    ...
                    if (wordArray[wordIndex]) {
                        for (let i = event.target.value.length; i < wordArray[wordIndex].length; i++) { 
                            document.getElementById(`${wordIndex}_${i}`).style.color = "#a8dadc"; 
                        }
                    }
                }
            }); 
            ...
            for (let i = 0; i < event.target.value.length; i++) { 
                if (document.getElementById(`${wordIndex}_${i}`))
                    if (event.target.value[i] === wordArray[wordIndex][i]) {
                        ...
                        document.getElementById(`${wordIndex}_${i}`).style.color = "#457b9d"; 

                    }
                    else {

                        // setting the character's color to red as it is a wrong character
                        document.getElementById(`${wordIndex}_${i}`).style.color = "#e63946";
